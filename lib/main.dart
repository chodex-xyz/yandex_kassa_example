...
final idempotenceKey = 'some_unique_idempotence_key' + DateTime.now().microsecondsSinceEpoch.toString();
  ...
  final paymentParameters = PaymentParameters(
      ...
      androidTestModeSettings: AndroidTestModeSettings(
          mockConfiguration: AndroidMockConfiguration(
              serviceFee: Amount(3.1415926), paymentAuthPassed: true, completeWithError: false, linkedCardsCount: 3)));
                  ...
                  if (tokenizationResult?.success == true)
                    OutlineButton(
                      ...
                      onPressed: () async {
                        ...
                        Request request = Request('POST', Uri.parse('https://payment.yandex.net/api/v3/payments'),
                            headers: Headers(),
                            body: json.encode({
                        ...
                        request.headers
                            .add('Authorization', "Basic " + base64Encode(utf8.encode("$shopId:$secretKey")));
                        request.headers.add('Content-Type', 'application/json');
                        Response response = await httpClient.send(request);
                        paymentResult = json.decode(await response.readAsString());
